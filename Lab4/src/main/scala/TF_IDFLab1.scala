    ...
    ddmd.take(5).foreach(f => {
      ...
      try {
        if (modelFolder.exists()) {
          ...
          for ((synonym, cosineSimilarity) <- synonyms) {
            //print("Synonums for " + f._1 + " " + s"$synonym $cosineSimilarity")
            println("Synonums for " + f._1 + " " + s"$synonym")
        ...
        println("ontologies = " + medWordls.collect().size)
          //.flatten.foreach( t => {
          //println("collect ontologies for high TFIDF medical word " + t)
          //medwv.write("medwords " + t + "\n")} )
      //})
      }
      catch
      {case e: Exception =>
