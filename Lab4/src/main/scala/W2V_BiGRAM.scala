...
object W2V_BiGRAM {
    def main(args: Array[String]): Unit = {
      ...
      val modelFolder = new File("W2V\\PerkinsonBigramModel")

      var s:String=""

      if (modelFolder.exists()) {

        val sameModel = Word2VecModel.load(sc, "W2V\\PerkinsonBigramModel")
        ...
        compWords.collect().foreach(w => {
          ...
          // Save and load model
          if(!modelFolder.exists()) {
            model.save(sc, "W2V/PerkinsonBigramModel")
          }
        })
      }
      val pw = new PrintWriter(new File("output/W2V_bigramMed.txt"))
      pw.write(s)
      pw.close()
