...
object BioNLP
{
  ...
  def main(args: Array[String]) {
    ...
    val pw = new PrintWriter(new File("output1/medicalwords.txt" ))
    val pw1 = new PrintWriter(new File("output1/medlist.txt" ))
      ...
      for (line <- Source.fromFile(Inputfile).getLines) {
        ...
        //lines.foreach(l => if (l.length() > 0) pw.write(l+ "\n") )
        val lines1 = lines.filter( line => line.contains("Gene") || line.contains("Disease")
          || line.contains("Chemical") || line.contains("Mutation"))

        val words = lines1.flatMap(word => {word.split("\t").drop(3).dropRight(1)}).toArray
        ...
        for (i <- 0 until words.length)
         {
           ...
           if (words(i).equals("Gene") ||  words(i).equals("Disease") ||
             words(i).equals("Chemical") ||  words(i).equals("Mutation")) {
             pw.write( "," + words(i) + "\n")
             pw1.write("\n")
    ...
    val pws = new PrintWriter(new File("output1/subclasses.txt" ))
    val pwc = new PrintWriter(new File("output1/classes.txt" ))

    val mdc1 = sc.textFile("output1/medlist.txt")
    val mdc2 = mdc1.distinct()
    mdc2.map(l => l).collect().foreach(l => pwc.write(l + "\n"))
    pwc.close()
    val md1 = sc.textFile("output1/medicalwords.txt")
    val md2 = md1.distinct()
     md2.map(l => l.split(",")).collect().foreach( l => pws.write(l(1)+ "," +  l(0) + "\n"))
    pws.close()
